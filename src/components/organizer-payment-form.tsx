...
export function OrganizerPaymentForm({ defaults }: { defaults: OrganizerPaymentFormDefaults }) {
  ...
  useEffect(() => {
    setQrUrl(defaults.payoutQrImageUrl);
  }, [defaults.payoutQrImageUrl]);
    ...
    <form action={formAction} className="space-y-6">
      ...
      <div className="border-t border-border pt-4">
        ...
        <p className="mt-1 text-xs text-fg-hint">
          แสดงในหน้าชำระเงินของผู้จองควบคู่กับ QR — เหมาะกับการโอนผ่านเลขบัญชีหรือสแกน QR จากแอปธนาคารของคุณ
        </p>
      ...
      <button type="submit" disabled={pending || uploading} className="jad-btn-primary h-12 w-full sm:w-auto">
